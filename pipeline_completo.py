# -*- coding: utf-8 -*-
"""Pipeline completo.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uf4ZK3tXpYMQJhSpac4DDlHZIxUgiVel
...
def distorcao_diametro(diametro, FOV, diametro_real, matrix_dim):
    """
...
#-------------------------------------------------------------------------------
#FUNÇÃO PARA CÁLCULO DA RELAÇÃO SINAL RUÍDO - UM CANAL
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#FUNÇÃO PARA CÁLCULO DA RELAÇÃO SINAL RUÍDO - Multicanal
#-------------------------------------------------------------------------------
...
def SNR_mult(imagem_data1, imagem_data2, mostrar_plots=False, multicanal = False):
    ...
    for i in range(imagem_data1.shape[2]):
        imagem_snr1 = imagem_data1[:,:,i]
        imagem_snr2 = imagem_data2[:,:,i]

        imagem_snr = imagem_snr1 - imagem_snr2  #Vai sobrar apenas o ruído.
        mask_otsu, t_otsu = maskotsu(imagem_snr)
        imagem_snr = 255 * imagem_snr/np.max(imagem_snr) if np.max(imagem_snr) > 0 else imagem_snr #Normalização
        S_MROI = (mask_otsu * imagem_snr1 + mask_otsu * imagem_snr2)/2 #Apenas região de sinal
...
def distribuicao():
    """
    Objetivo: A função está atualmente vazia e tem o objetivo de ser preenchida posteriormente para análises de distribuição de ruído.

    Entrada:
        - Nenhuma.

    Saída:
        - A função retorna uma string "adicionar", indicando que a implementação da lógica ainda está pendente.
    """

    return "adicionar"

#-------------------------------------------------------------------------------
#SALVANDO OS RESULTADOS DE FORMA ORGANIZADA PARA AVALIAÇÃO - UM CANAL
#-------------------------------------------------------------------------------

def pipeline_completo_single(imagem_path, ruido_path, FOV=50, diametro_real=25, matrix_dim=256,
...
#-------------------------------------------------------------------------------
#SALVANDO OS RESULTADOS DE FORMA ORGANIZADA PARA AVALIAÇÃO - MULTICANAL
#-------------------------------------------------------------------------------

#def pipeline_completo_mult(imagem_path1, imagem_path2,  ruido_path1, ruido_path2, FOV=50, diametro_real=25, matrix_dim=256, dim_roi=50, mostrar_plots=False, gerar_videos=False):

def pipeline_completo_mult(imagem_path1,  ruido_path1, FOV=50, diametro_real=25, matrix_dim=256,
                      dim_roi=50, mostrar_plots=False, gerar_videos=False):
    """
    Objetivo: Executar um pipeline completo de análise de qualidade de imagens de ressonância magnética, desde a leitura dos dados até o cálculo e compilação de métricas de qualidade (SNR, distorção, esfericidade, etc.).
    ...
    #ETAPA 1: LEITURA DAS IMAGENS
    print("ETAPA 1: LEITURA DAS IMAGENS")
    imagem1, ruido1, imagem_data1, imagem_aff1, imagem_hdr1, ruido_data1, ruido_aff1, ruido_hdr1 = Leituraimagens(imagem_path1, ruido_path1)
    imagem, ruido, imagem_data, imagem_aff, imagem_hdr, ruido_data, ruido_aff, ruido_hdr = Leituraimagens(imagem_path1, ruido_path1)

    print("DADOS DAS IMAGENS:")
    Dadosimagens(imagem1, ruido1, imagem_data1, imagem_aff1, imagem_hdr1)
    #Dadosimagens(imagem2, ruido2, imagem_data2, imagem_aff2, imagem_hdr2)

    #Linha temporária, apenas para rodar o código:
    imagem_data2 = imagem_data.copy()
    ...
    if mostrar_plots or gerar_videos:
        ...
        if mostrar_plots:
            plotagemimagens(imagem_data1)

        if gerar_videos:
            video_original = videoimagens(imagem_data1)
    else:
        print("="*80)
        print("ETAPA 2 PULADA: VISUALIZAÇÃO DAS IMAGENS")
    ...
    print("="*80)
    print("ETAPA 3: ANÁLISE DO RUÍDO")
    std_ruido, mean_ruido, img_noise = perfilruidoimagem(ruido_data1)

    dim = 50
    todas_df = []
    df_s = []

    for i in np.arange(0,imagem_data1.shape[2],1):
        roi1, roi2, roi3, roi4 = noise_regions(imagem_data[:,:,i], [0,0],[255-dim,0],[0,255-dim],[255-dim,255-dim], dim, plotar=False)
        df_estatisticas, roi = resumo_estatistica(roi1, roi2, roi3, roi4)
        todas_df.append(df_estatisticas)
        df_estacionario = teste_ruido_estacionario(roi1, roi2, roi3, roi4)
    ...
    print("="*80)
    print("ETAPA 4: FILTRAGEM DAS IMAGENS - Filtro Gaussinano")
    img_filtrada = filtragemimagens(imagem_data1)
    ...
    print("="*80)
    print("ETAPA 6: DETECTANDO CÍRCULOS - Transformada de Hough")
    circulos, diametros = detect_circles_in_slices(imagem_data1)
    ...
    # 7.2 - SNR
    print("2) SNR")
    df_snr = SNR_mult(imagem_data1, imagem_data2, mostrar_plots=False)

    # 7.3 - Homogeneidade e Contraste
    print("3) Homogeneidade e Contraste")
    df_h_c = calc_homogenidade_contraste(imagem_data1)
    ...
    # 7.6 - Coeficiente de Variação
    print("6) Coeficiente de Variação")
    df = calc_CV(mask_otsu,imagem_data1)
...
*   imagem_path = "/content/drive/MyDrive/Solenoide/Image_Phantom.nii"
*   ruido_path = "/content/drive/MyDrive/Solenoide/Noise.nii"

--------------------------------------------------------------------------

Utilizando os pipelines para um canal e para multicanais:


*   **pipeline_completo_single**(imagem_path, ruido_path, FOV=50, diametro_real=25,matrix_dim=256, dim_roi=50, mostrar_plots=True, gerar_videos=True).
*   df_final, pd_diametro = **pipeline_completo_single**(imagem_path, ruido_path, FOV=50, diametro_real=25, matrix_dim=256, dim_roi=50, mostrar_plots=True, gerar_videos=True).
*   **pipeline_completo_mult**(imagem_path1, imagem_path2,  ruido_path1, ruido_path2 FOV=50, diametro_real=25, matrix_dim=256,dim_roi=50, mostrar_plots=False, gerar_videos=False).
*   df_final, pd_diametro = **pipeline_completo_mult**(imagem_path1, imagem_path2,  ruido_path1, ruido_path2, FOV=50, diametro_real=25, matrix_dim=256,dim_roi=50, mostrar_plots=False, gerar_videos=False).
"""

imagem_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/milipede/Image_Phantom.nii"
ruido_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/milipede/Noise.nii"

df_final, pd_diametro = pipeline_completo_single(imagem_path, ruido_path)
#FOV=50, diametro_real=25, matrix_dim=256, dim_roi=50, mostrar_plots=True, gerar_videos=True, multicanal = False

df_final

imagem_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/8-channel coil/Image_Phantom.nii"
ruido_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/8-channel coil/Noise.nii"

df_final2, pd_diametro2 = pipeline_completo_mult(imagem_path, ruido_path,FOV=50, diametro_real=25, matrix_dim=256,dim_roi=50, mostrar_plots=False, gerar_videos=False)

!python --version

pd_diametro2

imagem_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/Solenoide/Image_Phantom.nii"
ruido_path = "/content/drive/MyDrive/UFABC/Quadrimestres - Pós graduação/Projetos/IC - Ressonância Magnética /Data_ICSarah/Data_ICSarah/Solenoide/Noise.nii"

df_final3, pd_diametro3 = pipeline_completo_mult(imagem_path, ruido_path,FOV=50, diametro_real=25, matrix_dim=256,dim_roi=50, mostrar_plots=False, gerar_videos=False)
